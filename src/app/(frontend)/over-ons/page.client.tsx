...
export default function OverOnsClient({ pageData }: OverOnsClientProps) {
  ...
  const hero = pageData?.hero || {
    title: 'WIE ZIJN WIJ',
    description: 'Al meer dan 25 jaar is TitanBrekers dé specialist in professioneel sloopwerk. Met passie, vakmanschap en moderne apparatuur maken wij ruimte voor de toekomst.',
  }
  ...
  const values = pageData?.values || [
    { icon: 'Shield', title: 'Veiligheid', description: 'Veiligheid staat altijd voorop. Wij werken volgens VCA** en hanteren de hoogste veiligheidsnormen op al onze projecten.' },
    { icon: 'Target', title: 'Kwaliteit', description: 'Wij leveren kwaliteitswerk, op tijd en binnen budget. Onze klanten kunnen rekenen op professionele uitvoering.' },
    { icon: 'Heart', title: 'Duurzaamheid', description: 'Met 98% recycling van sloopafval dragen wij bij aan een circulaire economie en een schonere toekomst.' },
    { icon: 'Users', title: 'Vakmanschap', description: 'Ons team bestaat uit ervaren vakmensen die trots zijn op hun werk en altijd streven naar het beste resultaat.' },
  ]
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div>
              <h2 className="font-display text-4xl mb-6 text-foreground">
                {story.title.split(' ').slice(0, -1).join(' ')} <span className="text-gradient">{story.title.split(' ').slice(-1)}</span>
              </h2>
            ...
            <div className="relative">
              <div className="grid grid-cols-2 gap-4">
                {stats.map((stat: any, idx: number) => (
                  <div key={idx} className={`card-industrial p-8 text-center ${idx % 2 === 1 ? 'mt-8' : ''}`}>
